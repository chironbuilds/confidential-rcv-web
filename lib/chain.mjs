 ...
 * or submitting anything. Building the mint statement needs no secret key at all (see mint.mjs --
 * its balance-proof signature is generated from an all-zero placeholder, not a real one), so this
 * can safely run against ANY future signer, not just this server's own account: the browser is
 * meant to hand these straight to `window.tari`'s `{ kind: "instructions" }` and let the
 * *connected wallet* (Sapient, Tari Universe, whichever) sign and pay the fee from its own
 * account -- the wallet's own `execute()` already knows how to register inputs and retry on
 * newly-discovered substates, so this deliberately returns bare instructions and a `maxFee`, no
 * pre-resolved inputs of its own to get out of sync with.
 */
export async function prepareInitiateElection(ctx, cfg, params) {
  ...
  const methodLiteral = tallyMethod === 'stv' ? '820180' : '820080';
